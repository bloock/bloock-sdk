...
import { BjjIssuerKey, BjjSigner, Bloock, IssuerKeyArgs } from "../../dist/index";
import { Credential, IdentityV2Client, KeyClient, KeyProtectionLevel, KeyType, ManagedKeyParams, IssuerKey, ProofType } from "../../dist";

function initSdk() {
    const apiKey = "tFD-hh1QYTj1TQEp3LulhHAredSkekobuuZI8vduysc7sx2RZTdpnX6A5FSQuSvT";
...
describe("Identity V2 Tests", () => {
    ...
    test("test identity end to end", async () => {
        ...
        let getNotFoundIssuerDid = await identityClient.getIssuerByKey(notFoundIssuerKey)
        expect(getNotFoundIssuerDid).toStrictEqual("")

        let issuers = await identityClient.getIssuerList()
        expect(issuers).toBeTruthy()

        let proofType = [ProofType.INTEGRITY_PROOF_TYPE, ProofType.SPARSE_MT_PROOF_TYPE]

        const schema = await identityClient
            .buildSchema("Driving License", drivingLicenseSchemaType, "1.0", "driving license schema", issuer)
            ...
            .addDecimalEnumAttribute("Precision wheels", "precision_wheels", "precision whels", true, [1.10, 1.20, 1.30])
            .build();
        expect(schema.id).toBeTruthy()

        const receipt = await identityClient.buildCredential(schema.id, issuer, holderDid, expiration, 0)
        ...
        const stateReceipt = await identityClient.buildIssuerStatePublisher(issuer)
            .withSigner(new BjjSigner(keyBjj))
            .build()
        expect(stateReceipt.txHash).toBeTruthy()

        expect(await identityClient.buildIssuerStatePublisher(issuer)
            .withSigner(new BjjSigner(keyBjj))
            .build()).toThrowError()
