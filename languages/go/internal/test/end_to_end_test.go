...
func TestEndToEnd(t *testing.T) {
	...
	t.Run("E2E using all the builders", func(t *testing.T) {
		...
		record, err := client.NewRecordBuilderFromString("Hello world").Build()
		...
		record, err = client.NewRecordBuilderFromBytes([]byte{1, 2, 3, 4, 5}).Build()
		require.NoError(t, err)
		hash, err = record.GetHash()
		require.NoError(t, err)
		...
		record, err = client.NewRecordBuilderFromHex("1234567890abcdef").Build()
		require.NoError(t, err)
		hash, err = record.GetHash()
		require.NoError(t, err)
		...
		record, err = client.NewRecordBuilderFromJSON("{\"hello\":\"world\"}").Build()
		require.NoError(t, err)
		hash, err = record.GetHash()
		require.NoError(t, err)
		...
		record, err = client.NewRecordBuilderFromRaw(
		...
		recordWithMultipleSignatures, err := client.
