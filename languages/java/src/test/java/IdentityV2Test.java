...
public class IdentityV2Test {
    ...
    static void beforeAll() {
        ...
        String apiHost = "https://api.bloock.dev";
    ...
    void endToEnd() throws Exception {
        IdentityClientV2 identityClient = new IdentityClientV2(apiManagedHost);
        ...
        assertEquals(issuer, getNotFoundIssuerDid);

        ArrayList<String> issuers = (ArrayList<String>) identityClient.getIssuerList();
        ...
        Schema schema = identityClient
                ...
                .addDecimalEnumAttribute("Precision wheels", "precision_wheels", "precision wheels", true, doubleList)
                .build();
        assertNotNull(schema.getId());

        CredentialReceipt receipt = identityClient.buildCredential(schema.getId(), issuer, holderDid, expiration, 0)
        ...
        IssuerStateReceipt stateReceipt = identityClient.buildIssuerStatePublisher(issuer)
                .withSigner(new BjjSigner(new SignerArgs(keyBjj))).build();
        assertNotNull(stateReceipt.getTxHash());

        assertThrows(RuntimeException.class, () -> {
            identityClient.buildIssuerStatePublisher(issuer).withSigner(new BjjSigner(new SignerArgs(keyBjj))).build();
            throw new RuntimeException("This is an intentional exception.");
        });
