...
class TestE2E(unittest.TestCase):
    ...
    def test_e2e_with_all_builders(self):
        ...
        hash = record.get_hash()
        self.assertEqual(hash, '7d87c5ea75f7378bb701e404c50639161af3eff66293e9f375b5f17eb50476f4')
        records.append(hash)

        record = RecordBuilder.from_hex('1234567890abcdef').build()
        hash = record.get_hash()
        self.assertEqual(hash, 'ed8ab4fde4c4e2749641d9d89de3d920f9845e086abd71e6921319f41f0e784f')
        records.append(hash)

        record = RecordBuilder.from_json('{"hello":"world"}').build()
        hash = record.get_hash()
        self.assertEqual(hash, '586e9b1e1681ba3ebad5ff5e6f673d3e3aa129fcdb76f92083dbc386cdde4312')
        records.append(hash)

        record = RecordBuilder.from_string('Hello world 2').build()
        record = RecordBuilder.from_record(record).build()
        hash = record.get_hash()
        self.assertEqual(hash, '96d59e2ea7cec4915c415431e6adb115e3c0c728928773bcc8e7d143b88bfda6')
        records.append(hash)

        record = RecordBuilder.from_file(bytes([2, 3, 4, 5, 6])).build()
        hash = record.get_hash()
        self.assertEqual(hash, '507aa5dd7b2e52180b764db13c8289ed204109cafe2ef4e453366da8654dc446')
        records.append(hash)

        keys = self.client.generate_keys()

        record = RecordBuilder.from_string('Hello world 3')\
          .with_signer(EcsdaSigner(keys.private_key))\
          .build()

        keys = self.client.generate_keys()

        record_with_multiple_signatures = RecordBuilder.from_record(record)\
          .with_signer(EcsdaSigner(keys.private_key))\
          .build()
