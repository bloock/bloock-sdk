...
impl Blockchain for Web3 {
    fn validate_state(
        ...
        let http_provider = web3::transports::Http::new(&provider)
            .map_err(|e| InfrastructureError::Web3Error(e.to_string()))?;
        ...
        let result = contract.query(
            "getState",
            H256::from_str(&state).map_err(|e| InfrastructureError::Web3Error(e.to_string()))?,
            None,
            Options::default(),
            None,
        );

        let state: U256 = tokio::runtime::Builder::new_current_thread()
            .enable_all()
            .build()
            .map_err(|e| InfrastructureError::Web3Error(e.to_string()))?
