...
lazy_static! {
    ...
    static ref OID_HMAC_WITH_SHA1: ObjectIdentifier = as_oid(&[1, 2, 840, 113549, 2, 7]);
...
impl EncryptedContentInfo {
    ...
    pub fn from_safe_bags(safe_bags: &[SafeBag], password: &[u8]) -> Option<EncryptedContentInfo> {
        ...
        let params = Parameters::pbkdf2_sha256_aes256cbc(ITERATIONS as u32, &salt, &iv).ok()?;
        let params = Pbe2Params {
            der: params.to_der().ok()?,
        };

        let encrypted_content = pbes2_encrypt(&data, password, &params)?;
        let content_encryption_algorithm = AlgorithmIdentifier::Pbes2(params);
...
//     pub fn parse(r: BERReader) -> Result<Self, ASN1Error> {
//         let der = r.read_der()?;
//         let params = Parameters::from_der(der.as_slice()).unwrap();
//         Ok(Pbe2Params(params))
//     }
//     pub fn write(&self, w: DERWriter) {
//         let der = self.0.to_der().unwrap();
//         w.write_der(der.as_slice())
//     }
// }
...
impl AlgorithmIdentifier {
    ...
    pub fn decrypt_pbe(&self, ciphertext: &[u8], password: &[u8]) -> Option<Vec<u8>> {
        match self {
            ...
            AlgorithmIdentifier::Pbes2(param) => pbes2(ciphertext, password, &param),
    ...
    pub fn write(&self, w: DERWriter) {
        w.write_sequence(|w| match self {
            ...
            AlgorithmIdentifier::Pbes2(p) => {
                ...
                let params = Parameters::from_der(&p.der).unwrap();
                let der = params.to_der().unwrap();
                w.next().write_der(der.as_slice());
            }
...
impl PFX {
    ...
    pub fn new_with_cas(
        ...
        let encrypted_data = pbes2_encrypt(key_der, &password, &params)?;
        let key_bag_inner = SafeBagKind::Pkcs8ShroudedKeyBag(EncryptedPrivateKeyInfo {
            encryption_algorithm: AlgorithmIdentifier::Pbes2(params),
            encrypted_data,
        });
...
fn pbes2(data: &[u8], password: &[u8], params: &Pbe2Params) -> Option<Vec<u8>> {
    let password = decode_bmp_string(password)?;
    let params = Parameters::from_der(&params.der).unwrap();
    params.decrypt(password, data).ok()
}

fn pbes2_encrypt(data: &[u8], password: &[u8], params: &Pbe2Params) -> Option<Vec<u8>> {
    let password = decode_bmp_string(password)?;
    let params = Parameters::from_der(&params.der).unwrap();
    params.encrypt(password, data).ok()
}
...
fn decode_bmp_string(s: &[u8]) -> Option<Vec<u8>> {
    if s.len() % 2 != 0 {
        return None;
    }
    ...
    let l = s.len();
    if l >= 2 && s[l - 1] == 0 && s[l - 2] == 0 {
        result = s[0..(l - 2)].to_vec();
    }

    let mut s = Vec::with_capacity(s.len() / 2);

    while !result.is_empty() {
        s.push(u16::from(result[0]) << 8 | u16::from(result[1]));
        result = result[2..].to_vec();
    }

    let utf16_chars: Vec<u16> = s;
    let decoded_string = String::from_utf16(&utf16_chars).ok()?;

    Some(decoded_string.as_bytes().to_vec())
}
