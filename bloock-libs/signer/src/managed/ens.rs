...
impl ManagedEnsSigner {
    pub fn new(key: ManagedKey, api_host: String, api_key: String) -> Self {
        Self {
            managed_key: key,
            api_host,
            api_key,
    ...
    pub fn new_boxed(key: ManagedKey, api_host: String, api_key: String) -> Box<Self> {
        Box::new(Self::new(key, api_host, api_key))
    }
}

#[async_trait(?Send)]
impl Signer for ManagedEnsSigner {
    async fn sign(&self, payload: &[u8]) -> Result<Signature> {
        let ecdsa_signer = ManagedEcdsaSigner::new(
            self.managed_key.clone(),
            None,
            self.api_host.clone(),
            self.api_key.clone(),
        );
