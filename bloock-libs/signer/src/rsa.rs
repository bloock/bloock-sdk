...
use bloock_hasher::sha256::Sha256 as BloockSha256;
...
impl BloockSigner for RsaSigner {
    async fn sign_local(&self, payload: &[u8], key: &Local) -> crate::Result<BloockSignature> {
        let local = match key {
            Local::Key(k) => k.clone(),
    ...
    async fn sign_managed(&self, payload: &[u8], key: &Managed) -> crate::Result<BloockSignature> {
...
mod tests {
    ...
    async fn test_sign_local_certificate_ok() {
        ...
        let signature = signer
            .sign_local(string_payload.as_bytes(), &local_cert.clone().into())
            .await
            .unwrap();
