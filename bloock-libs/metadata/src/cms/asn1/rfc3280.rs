    ...
    pub fn encode_as(self, tag: Tag) -> impl Values {
        encode::sequence_as(
            ...
            (
                self.name_assigner
                    .as_ref()
                    .map(|name_assigner| name_assigner.clone().encode()),
                self.party_name.encode(),
            ),
        )
    }
...
impl DirectoryString {
    ...
    pub fn encode(self) -> impl Values {
        match self {
            Self::PrintableString(ps) => (Some(ps.encode()), None),
            Self::Utf8String(s) => (None, Some(s.encode())),
            _ => unimplemented!(),
        }
    }
...
impl Name {
    ...
    pub fn encode_as(self, tag: Tag) -> impl Values {
        match self {
            Self::RdnSequence(seq) => seq.encode_as(tag),
        }
    }
