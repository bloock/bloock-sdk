...
use {
    ...
    x509_certificate::{asn1time::*, rfc5280::*},
};
...
impl Deref for GeneralNames {
    type Target = Vec<GeneralName>;
...
impl DerefMut for GeneralNames {
...
impl IssuerSerial {
    pub fn encode(self) -> impl Values {
        encode::sequence((
            self.issuer.encode(),
            // self.serial.encode(),
            self.issuer_uid.map(|u| u.encode()),
        ))
    }
    ...
    pub fn encode_ref_as(&self, tag: Tag) -> impl encode::Values + '_ {
        encode::sequence_as(
            ...
            (
                ...
                self.serial.encode(),
                self.issuer_uid.as_ref().map(|u| u.encode()),
            ),
        )
    }
}
