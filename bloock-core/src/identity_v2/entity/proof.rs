...
impl Serialize for CredentialProof {
        ...
        S: serde::Serializer,
    {
        ...
        if let Some(bloock_proof) = self.bloock_proof.clone() {
            let mut proof = Value::from_str(&bloock_proof.clone())
            ...
            for proof in proof_array.iter() {
                if let Some(proof_type) = proof["type"].as_str() {
                    match proof_type {
                        ...
                        BLOOCK_PROOF_TYPE => {
                            bloock_proof = Some(proof.to_string());
                        }
